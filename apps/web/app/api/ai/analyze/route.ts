...
import { AiAnalyzeRequestSchema } from "@fillin/schemas";
import { analyzeForm } from "@/lib/ai-service";
...
export async function POST(req: NextRequest) {
  ...
  const rl = rateLimit(`analyze:${clientIp(req.headers)}`);
  if (!rl.ok) {
  ...
  try {
    ...
    const result = await analyzeForm(parsed.data);
